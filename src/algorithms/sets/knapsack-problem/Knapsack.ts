...
export class Knapsack
{
  ...
  weightLimit: number;
  possibleItems: KnapsackItem[];

  /**
   * @param {KnapsackItem[]} possibleItems
   * @param {number} weightLimit
   */
  constructor(possibleItems: KnapsackItem[], weightLimit: number)
  {
    this.selectedItems = [];
  ...
  {
    this.possibleItems = new MergeSort<KnapsackItem>({
  ...
  solveZeroOneKnapsackProblem()
  {
    ...
    const knapsackMatrix: number[][] = Array(numberOfRows).fill(null).map(() =>
      Array(numberOfColumns + 1).fill(null));
  ...
  get totalValue()
  {
    return this.selectedItems.reduce((accumulator, item) =>
      accumulator + item.totalValue, 0);
  }

  get totalWeight()
  {
    return this.selectedItems.reduce((accumulator, item) =>
      accumulator + item.totalWeight, 0);
  }
}
