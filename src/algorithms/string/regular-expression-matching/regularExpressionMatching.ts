 ...
 * @param pattern
 */
export function regularExpressionMatching(string: string, pattern: string)
{
  /*
    ...
    * b - - - - - -
   */
  const matchMatrix: boolean[][] = Array(string.length + 1).fill(null).map(() =>
    Array(pattern.length + 1).fill(null));
